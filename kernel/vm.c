...
// lab3 将设置的函数重新写一下，方便给指定的页表建立映射，之前就只有一个全局的内核页表，不需要，但现在需要给每个内核进程创建内核页表，所以需要独立出来
void kvm_map_pagetable(pagetable_t pgtbl) {
  ...
  // CLINT
  // kvmmap(pgtbl, CLINT, CLINT, 0x10000, PTE_R | PTE_W);//lab3.3修改，单独给全局内核页表映射
...
kvminit()
{
  ...
  kvmmap(kernel_pagetable, CLINT, CLINT, 0x10000, PTE_R | PTE_W);//lab3.3修改，单独给全局内核页表映射
 ...
 * create a direct-map page table for the kernel.
 */
/*被上面的kvminit+kvminit_newpgtbl+kvm_map_pagetable合并代替
void
kvminit()
{
...
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)//walk函数模拟了MMU，返回的是va对应的最低级page table的PTE）
{
  if(va >= MAXVA)
    panic("walk");
...
walkaddr(pagetable_t pagetable, uint64 va)
{
  ...
  pte = walk(pagetable, va, 0);//返回最低一级的pte
  if(pte == 0)
    return 0;
  if((*pte & PTE_V) == 0)
    return 0;
...
//lab3添加的打印页表的函数
int pgtblprint(pagetable_t pagetable, int depth) {//传入的pagetable是物理地址
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];//连续的512项
    if(pte & PTE_V) { // 如果页表项有效
      ...
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0){
        // this PTE points to a lower-level page table.
        uint64 child = PTE2PA(pte);//根据该页表的条目，找到对应的物理地址(即下一级的页表，直到最后找到真正的物理地址)
